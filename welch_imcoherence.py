    ...
    def update(self):
        ...
        for elec1 in range(data.shape[1]):
          ...
          for elec2 in range(data.shape[1]):
            ...
            f,Pxx = welch(data[:,elec1], fs=rate, **self._kwargs)
            f2,Pxx2 = welch(data[:,elec2], fs=rate, **self._kwargs)
            fxy, ImCoh = signal.csd(data[:,elec1], data[:,elec2], fs=rate, window='hann',  **self._kwargs)
            Cxy_MI = np.abs(np.imag(ImCoh))/(np.sqrt(Pxx*Pxx2))
